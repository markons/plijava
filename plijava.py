"""
================================================================================
 plijava.py  -  PL/I to Java Transpiler                              v2.00
================================================================================
...
--------------------------------------------------------------------------------
  ...
  Built-in functions
    ...
    length(s)                          -> s.length()
    trim(s)                            -> s.trim()
    lowercase(s) / uppercase(s)        -> s.toLowerCase() / .toUpperCase()
    repeat(s, n)                       -> s.repeat(n)
    reverse(s)                         -> new StringBuilder(s).reverse().toString()
    abs(x), sqrt(x), ceil(x), floor(x) -> Math.abs / sqrt / ceil / floor
    round(x), trunc(x), sign(x)        -> Math.round / cast / Integer.signum
    max(a,b), min(a,b)                 -> Math.max / Math.min
    date(), time()                     -> java.time formatted strings
...
--------------------------------------------------------------------------------
  ...
  - No multi-row / multi-column SQL.
  - get list() type detection relies on previously seen dcl output;
    may fail if dcl appears after get in the source.
  - ON conditions: only ENDFILE is implemented (v2.00).  ERROR, KEY,
    UNDEFINEDFILE etc. are not supported.

--------------------------------------------------------------------------------
 VERSION HISTORY
--------------------------------------------------------------------------------
  v2.00  2026-04-22  [Claude Code] Major language-coverage release.
                     Adds LEAVE / ITERATE / STOP statements, BY clause on
                     DO FROM/TO loops (positive and negative literal step),
                     INIT / INITIAL clause on scalar DCL declarations,
                     BIT(n) type mapped to Java boolean, ON ENDFILE handlers
                     (read wraps in null-check), and a built-in dispatch
                     table covering LENGTH, TRIM, LOWERCASE, UPPERCASE,
                     REPEAT, REVERSE, ABS, SQRT, CEIL, FLOOR, ROUND, TRUNC,
                     SIGN, MAX, MIN, DATE, TIME.  All changes generated by
                     Claude Code.
...
tokens = (
    ...
    # v2.00 additions
    'LEAVE', 'ITERATE', 'STOP', 'BY', 'INIT', 'BIT', 'ON', 'ENDFILE',
)
...
def p_variable_access(p):
    ...
    # v2.00: when this rule matches a call like `length(x)` or `max(a,b)` with
    # only ID/NUMBER args, the function-call rule never gets a chance — so we
    # intercept built-ins here too.
    name_lower = p[1].lower() if isinstance(p[1], str) else None
    if not is_array and name_lower in BUILTIN_FUNCS:
        spec = BUILTIN_FUNCS[name_lower]
        if len(p) == 7:
            ...
            args = []
        if spec['arity'] is None or len(args) == spec['arity']:
            p[0] = spec['emit'](args)
            logger.debug('end variable_access (builtin %s): %s', name_lower, p[0])
            return
...
def p_declaration(p):
    '''declaration : DCL id_list type_declaration
                   | DCL id_list type_declaration INIT LPAREN expression RPAREN
    ...
    if len(p) == 6:  # Record declaration
       ...
       # v2.00: detect optional INIT(<expr>) clause on scalar declarations
       init_value = None
       if len(p) == 8:  # DCL id_list type_declaration INIT ( expression )
           type_str = p[3] or "FIXED BIN"
           init_value = p[6]
           is_array = False
       elif len(p) == 5:  # array declaration
           type_str = p[4] or "FIXED BIN"
           is_array = True
       else:  # len(p) == 4: plain scalar declaration
           type_str = p[3] or "FIXED BIN"
           is_array = False
       ...
       for var in p[2]:  # id_list
           if is_array:
               ...
               if "BIT" in type_upper:
                   # v2.00: BIT(1) → boolean. Init '1'b → true, '0'b → false (or any literal '1'/'0').
                   if init_value is not None:
                       iv = str(init_value).strip().strip('()').strip().strip('"').lower()
                       bool_val = "true" if iv in ("1", "1b") else "false"
                       decls.append(f"boolean {var} = {bool_val};")
                   else:
                       decls.append(f"boolean {var} = false;")
               elif "CHAR" in type_upper:
                   if init_value is not None:
                       decls.append(f'String {var} = {init_value};')
                   else:
                       decls.append(f'String {var} = "";')
               elif "FIXED BIN" in type_upper:
                   precision = variable_length(type_str)
                   java_t = "long" if precision is not None and precision > 15 else "int"
                   if init_value is not None:
                       decls.append(f"{java_t} {var} = {init_value};")
                   else:
                       decls.append(f"{java_t} {var} = 0;")
               else:
                   if init_value is not None:
                       decls.append(f"int {var} = {init_value};")
                   else:
       ...
       if not is_array:
           for var in p[2]:
               type_upper = type_str.upper()
               if "BIT" in type_upper:
                   declared_var_types[var] = "boolean"
               elif "CHAR" in type_upper:
...
def p_type_declaration(p):
    ...
    if len(p) == 6:  # FIXED BIN(n)
        ...
        p[0] = f"CHAR({p[3]})"
    elif p[1].lower() == 'bit':   # BIT(n) — v2.00, mapped to Java boolean
        p[0] = f"BIT({p[3]})"
...
# v2.00: ON ENDFILE — registers a handler for the named file.
# The handler is consulted by p_read_file to wrap readLine() in a null check.
file_endfile_handlers = {}  # fname -> handler statement string

def p_on_endfile_statement(p):
    '''on_endfile_statement : ON ENDFILE LPAREN CHAR_CONST RPAREN statement'''
    fname = p[4].replace('"', '')
    handler = p[6]
    if isinstance(handler, list):
        handler = "\n".join(handler)
    file_endfile_handlers[fname] = handler
    # Emit a trace comment so the generated Java still reflects the source line.
    p[0] = f"// on endfile('{fname}') registered"
...
def _split_args(arg_str):
    """Split a parameter_list string ('a, b, c') on top-level commas only."""
    out, depth, buf = [], 0, []
    for ch in arg_str:
        if ch == '(':
...
def p_expression_function_call(p):
    ...
    # v2.00: built-in dispatch — intercept well-known PL/I built-ins and
    # rewrite them to the equivalent Java expression instead of letting them
    # fall through to a literal `name(args)` call.
    if len(p) == 5:
        args = _split_args(p[3])
    else:
        args = []
    ...
    if name in BUILTIN_FUNCS:
        ...
        if expected is None or len(args) == expected:
            p[0] = spec['emit'](args)
            logger.debug('end expression_function_call (builtin %s): %s', name, p[0])
            return
...
# v2.00: PL/I built-in function table.
# Each entry: 'arity' = expected argument count (None = any), 'emit' = lambda(args) -> str
BUILTIN_FUNCS = {
    # ---- string built-ins ----
    'length':    {'arity': 1, 'emit': lambda a: f"({a[0]}).length()"},
    'trim':      {'arity': 1, 'emit': lambda a: f"({a[0]}).trim()"},
    'lowercase': {'arity': 1, 'emit': lambda a: f"({a[0]}).toLowerCase()"},
    'uppercase': {'arity': 1, 'emit': lambda a: f"({a[0]}).toUpperCase()"},
    'repeat':    {'arity': 2, 'emit': lambda a: f"({a[0]}).repeat((int)({a[1]}))"},
    'reverse':   {'arity': 1, 'emit': lambda a: f"new StringBuilder({a[0]}).reverse().toString()"},
    # ---- math built-ins ----
    'abs':       {'arity': 1, 'emit': lambda a: f"Math.abs({a[0]})"},
    'sqrt':      {'arity': 1, 'emit': lambda a: f"(long)Math.sqrt({a[0]})"},
    'ceil':      {'arity': 1, 'emit': lambda a: f"(long)Math.ceil({a[0]})"},
    'floor':     {'arity': 1, 'emit': lambda a: f"(long)Math.floor({a[0]})"},
    'round':     {'arity': 1, 'emit': lambda a: f"Math.round((double)({a[0]}))"},
    'trunc':     {'arity': 1, 'emit': lambda a: f"(long)({a[0]})"},
    'sign':      {'arity': 1, 'emit': lambda a: f"Integer.signum((int)({a[0]}))"},
    'max':       {'arity': 2, 'emit': lambda a: f"Math.max({a[0]}, {a[1]})"},
    'min':       {'arity': 2, 'emit': lambda a: f"Math.min({a[0]}, {a[1]})"},
    # ---- date/time built-ins (no args) ----
    'date':      {'arity': 0, 'emit': lambda a: 'java.time.LocalDate.now().format(java.time.format.DateTimeFormatter.ofPattern("yyMMdd"))'},
    'time':      {'arity': 0, 'emit': lambda a: 'java.time.LocalTime.now().format(java.time.format.DateTimeFormatter.ofPattern("HHmmssSSS"))'},
}
...
def get_variable_type(variable_name, declarations):
    ...
    pattern = r"(int|String|long|boolean)\s+(\w+)\s*="
...
def p_do_from_to_statement(p):
    '''do_from_to_statement : DO ID ASSIGN expression TO expression SEMICOLON statement do_end
                            ...
                            | DO ID ASSIGN expression TO expression BY expression SEMICOLON statement do_end
                            | DO ID ASSIGN expression TO expression BY expression SEMICOLON statement_list do_end'''
    ...
    # v2.00: optional BY clause (step). If absent, step = 1.
    has_by = len(p) == 12
    if has_by:
        step_expr = p[8]
        body_idx = 10
    ...
    if step_expr is None:
        ...
        # If step is a negative integer literal, flip comparison and emit i -= |step|.
        step_str = str(step_expr).strip().strip('()')
        try:
            step_int = int(step_str)
            if step_int < 0:
                p[0] = (f"for ({loop_variable} = {start_expr}; {loop_variable} >= {end_expr}; "
                        f"{loop_variable} -= {-step_int}) {{\n{loop_body}")
            elif step_int > 0:
                p[0] = (f"for ({loop_variable} = {start_expr}; {loop_variable} <= {end_expr}; "
                        f"{loop_variable} += {step_int}) {{\n{loop_body}")
            else:
                # step 0 would loop forever; emit it but it's the user's bug.
                p[0] = (f"for ({loop_variable} = {start_expr}; {loop_variable} <= {end_expr}; "
                        f"{loop_variable} += 0) {{\n{loop_body}")
        except (TypeError, ValueError):
            # Non-literal step: assume positive direction.
            p[0] = (f"for ({loop_variable} = {start_expr}; {loop_variable} <= {end_expr}; "
                    f"{loop_variable} += ({step_expr})) {{\n{loop_body}")
...
def p_read_file(p):
    ...
    fname = p[4].replace('"', "")
    var_name = p[8]
    handler = file_endfile_handlers.get(fname)
    if handler:
        # Wrap the read in a null check; on EOF run the registered handler.
        p[0] = (f"{{ String _eof_{fname} = in_{fname}.readLine(); "
                f"if (_eof_{fname} == null) {{ {handler} }} "
                f"else {{ {var_name} = _eof_{fname}; }} }}")
    else:
